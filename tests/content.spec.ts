...
for (const { lang, url, cvSuffix } of locales) {
	test.describe(`[${lang}] content integrity`, () => {
		...
		test('Hero: name, role text, Explore button, CV download visible', async ({
			page,
		}) => {
			...
			const cvLink = page.locator(
				`a[href="/files/Raffaele-Valenti-CV-${cvSuffix}.pdf"]`,
			);
		...
		test('Experience: at least one card with role and company', async ({
			page,
		}) => {
		...
		test('Projects: at least one card with title and description', async ({
			page,
		}) => {
		...
		test('Skills: section title visible, skill category heading present', async ({
			page,
		}) => {
		...
		test('Education: at least one entry with degree and institution', async ({
			page,
		}) => {
		...
		test('Contact: email link and social links present', async ({ page }) => {
			...
			await expect(
				page.locator('#contact a[aria-label="GitHub"]'),
			).toBeVisible();
			await expect(
				page.locator('#contact a[aria-label="LinkedIn"]'),
			).toBeVisible();
		});
