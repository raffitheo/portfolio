...
test.describe('Desktop navigation', () => {
	...
	test('scroll-to-top button appears after 500px and scrolls back to top', async ({
		page,
	}) => {
	...
	for (const { href, lang: targetLang } of [
		...
		test(`language switcher: click ${targetLang} navigates to ${href}`, async ({
			page,
		}) => {
...
test.describe('Mobile navigation', () => {
	...
	test('Escape closes mobile menu and returns focus to toggle', async ({
		page,
	}) => {
		await page.click('#mobile-menu-toggle');
	...
	for (const section of NAV_SECTIONS) {
		test(`mobile nav click scrolls to #${section} and closes menu`, async ({
			page,
		}) => {
			await page.click('#mobile-menu-toggle');
