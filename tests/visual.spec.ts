...
test.describe('Full page visual regression', () => {
	for (const { lang, url } of [
		...
		test(`[${lang}] full page`, async ({ page }) => {
			...
			await expect(page).toHaveScreenshot(`full-page-${lang}.png`, {
				fullPage: true,
			});
		});
	}
...
test.describe('Section visual regression', () => {
	...
	test('Navbar at top - desktop', async ({ page }) => {
		...
		await page.goto('/');
		await page.waitForLoadState('networkidle');
		await expect(page.locator('#desktop-nav')).toHaveScreenshot(
			'navbar-top-desktop.png',
		);
	});
	...
	test('Navbar at top - mobile', async ({ page }) => {
		...
		await page.goto('/');
		await page.waitForLoadState('networkidle');
		await expect(page.locator('#mobile-nav')).toHaveScreenshot(
			'navbar-top-mobile.png',
		);
	});
